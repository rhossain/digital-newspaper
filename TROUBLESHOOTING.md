...
In local development, this can happen more often because the Angular dev proxy sends requests server-to-server. Imunify360 may classify those requests as automation and return a JSON denial payload instead of the real newspaper data.
...
If path allowlisting is not available, whitelist the trusted admin/deployment IPs that access the API.
