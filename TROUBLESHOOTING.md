...
## WordPress API Blocked By Imunify360 Bot Protection

### Problem
The Angular app shows an error like:

```text
Invalid newspaper API response: Access denied by Imunify360 bot-protection. IPs used for automation should be whitelisted
```

or:

```text
WordPress did not return a login token. The request is likely being blocked by Imunify360 before it reaches the Digital Newspaper plugin.
```

In DevTools this can look confusing because the request may show `200 OK`, but the body is not the plugin login response. A real login response includes a `token`; the Imunify360 block response includes only a `message`.

The affected endpoint is usually:

```text
https://epaper.dailysangram.com/wp/wp-json/digital-newspaper/v1/data
```

### Why This Happens
This response is generated by the hosting security layer before the request reaches the Digital Newspaper WordPress plugin.

In local development, this happened more often when the Angular dev proxy sent requests server-to-server. The app now calls WordPress directly from the browser in `environment.ts`, and `ng serve` no longer attaches `proxy.conf.js`.

The WordPress plugin also no longer starts automatic background social-cache warming after saves. The `/warm-cache` endpoint still exists for deliberate admin use, but it requires the same credentialed token as other admin actions.

Before the latest fix, Angular could accidentally treat this denial JSON as an empty newspaper dataset. Now the app rejects it clearly so bad host responses cannot overwrite or hide production data silently.

### Hosting Fix If The Server Still Blocks Requests
In Hostinger / Imunify360 / server security settings, allow the Digital Newspaper REST API paths:

```text
/wp/wp-json/digital-newspaper/v1/*
/wp/wp-json/wp/v2/media*
```

If path allowlisting is not available, whitelist the trusted admin/deployment IPs that access the API. This is only needed if Imunify360 still blocks direct browser requests before they reach WordPress.

You can confirm the block with:

```bash
curl -i -X POST 'https://epaper.dailysangram.com/wp/wp-json/digital-newspaper/v1/auth/login' \
   -H 'Content-Type: application/x-www-form-urlencoded;charset=UTF-8' \
   --data 'username=__probe__&password=__probe__'
```

If the response body says `Access denied by Imunify360 bot-protection`, the request is not reaching WordPress or the Digital Newspaper plugin yet. Updating the plugin, changing permalinks, or changing Angular CORS settings will not fix that server-layer block.

### WordPress CORS Check
If using the Angular app from localhost, also verify the plugin settings include the local origin:

```text
http://localhost:4200
```

For production, the app should be served from the same hostname as WordPress.

### Emergency Client-Side Safety
The Angular service now keeps an emergency browser-local draft for normal structured data snapshots. If the live API is blocked, the app can fall back to that local draft instead of immediately showing an empty state, giving the admin a chance to export a backup.

This is not a replacement for WordPress persistence. The real fix is still to unblock the WordPress REST API at the hosting/security layer.
